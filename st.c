...
void tscrollup(int orig, int n)
{
    ...
    /* Ensure that lines are allocated */
    for (int i = term.row; i < term.row + n; i++) {
        TLINE(i) = ensureline(TLINE(i));
    }
    ...
    for (int i = term.row-1; i >term.bot; i--) {
        temp = TLINE(i);
        TLINE(i) = TLINE(i+n);
        TLINE(i+n) = temp;
    }
...
void tnewline(int first_col)
{
    ...
    tmoveto(first_col ? 0 : term.cur.x, y);
}
...
{
    tmoveto(x, y + ((term.cur.state & CURSOR_ORIGIN) ? term.top: 0));
}
...
void tmoveto(int x, int y)
{
    ...
    if (term.cur.state & CURSOR_ORIGIN) {
        miny = term.top;
        maxy = term.bot;
    } else {
        miny = 0;
    ...
    term.cur.state &= ~CURSOR_WRAPNEXT;
    term.cur.x = LIMIT(x, 0, term.col-1);
    term.cur.y = LIMIT(y, miny, maxy);
}
...
tclearregion(int x1, int y1, int x2, int y2)
{
    ...
    for (y = y1; y <= y2; y++) {
        ...
        for (x = x1; x <= x2; x++) {
            ...
            if (selected(x, y))
                selclear();
            gp->fg = term.cur.attr.fg;
            gp->bg = term.cur.attr.bg;
            gp->mode = 0;
            gp->u = ' ';
        }
        L = (L + 1) % TSCREEN.size;
...
tdeletechar(int n)
{
    ...
    LIMIT(n, 0, term.col - term.cur.x);

    dst = term.cur.x;
    src = term.cur.x + n;
    size = term.col - src;
    line = TLINE(term.cur.y);

    memmove(&line[dst], &line[src], size * sizeof(Glyph));
    tclearregion(term.col-n, term.cur.y, term.col-1, term.cur.y);
}
...
void tinsertblank(int n)
{
    ...
    int dst = term.cur.x + n;
    int src = term.cur.x;
    int size = term.col - dst;
    Glyph *line = TLINE(term.cur.y);

    memmove(&line[dst], &line[src], size * sizeof(Glyph));
    tclearregion(src, term.cur.y, dst - 1, term.cur.y);
}

void tinsertblankline(int n)
{
    if (BETWEEN(term.cur.y, term.top, term.bot)) {
        tscrolldown(term.cur.y, n);
    }
}
    ...
    for (int i = 0; i < l; i++) {
        ...
        case 0:
            term.cur.attr.mode &= ~(
                ...
                ATTR_INVISIBLE  |
                ATTR_STRUCK     );
            term.cur.attr.fg = defaultfg;
            term.cur.attr.bg = defaultbg;
            break;
        case 1:
            term.cur.attr.mode |= ATTR_BOLD;
            break;
        case 2:
            term.cur.attr.mode |= ATTR_FAINT;
            break;
        case 3:
            term.cur.attr.mode |= ATTR_ITALIC;
            break;
        case 4:
            term.cur.attr.mode |= ATTR_UNDERLINE;
            break;
        case 5: /* slow blink */
            /* FALLTHROUGH */
        case 6: /* rapid blink */
            term.cur.attr.mode |= ATTR_BLINK;
            break;
        case 7:
            term.cur.attr.mode |= ATTR_REVERSE;
            break;
        case 8:
            term.cur.attr.mode |= ATTR_INVISIBLE;
            break;
        case 9:
            term.cur.attr.mode |= ATTR_STRUCK;
            break;
        case 22:
            term.cur.attr.mode &= ~(ATTR_BOLD | ATTR_FAINT);
            break;
        case 23:
            term.cur.attr.mode &= ~ATTR_ITALIC;
            break;
        ...
        default:
            if (BETWEEN(attr[i], 30, 37)) {
                term.cur.attr.fg = attr[i] - 30;
            } else if (BETWEEN(attr[i], 40, 47)) {
                term.cur.attr.bg = attr[i] - 40;
            } else if (BETWEEN(attr[i], 90, 97)) {
                term.cur.attr.fg = attr[i] - 90 + 8;
            } else if (BETWEEN(attr[i], 100, 107)) {
                term.cur.attr.bg = attr[i] - 100 + 8;
            } else {
    ...
    for (lim = args + narg; args < lim; ++args) {
        if (priv) {
            ...
            case 5: /* DECSCNM -- Reverse video */
                ...
                break;
            case 6: /* DECOM -- Origin */
                MODBIT(term.cur.state, set, CURSOR_ORIGIN);
                tmoveato(0, 0);
                break;
...
csihandle(void)
{
    ...
    case 'A': /* CUU -- Cursor <n> Up */
        DEFAULT(csiescseq.arg[0], 1);
        tmoveto(term.cur.x, term.cur.y-csiescseq.arg[0]);
        break;
    ...
    case 'a': /* HPR -- Cursor <n> Forward */
        DEFAULT(csiescseq.arg[0], 1);
        tmoveto(term.cur.x+csiescseq.arg[0], term.cur.y);
        break;
    ...
    case 'E': /* CNL -- Cursor <n> Down and first col */
        DEFAULT(csiescseq.arg[0], 1);
        tmoveto(0, term.cur.y+csiescseq.arg[0]);
        break;
    ...
    case '`': /* HPA */
        DEFAULT(csiescseq.arg[0], 1);
        tmoveto(csiescseq.arg[0]-1, term.cur.y);
        break;
    ...
    case 'J': /* ED -- Clear screen */
        switch (csiescseq.arg[0]) {
        case 0: /* below */
            tclearregion(term.cur.x, term.cur.y, term.col-1, term.cur.y);
            if (term.cur.y < term.row-1) {
                tclearregion(0, term.cur.y+1, term.col-1,
                        term.row-1);
            }
            break;
        case 1: /* above */
            if (term.cur.y > 0)
                tclearregion(0, 0, term.col-1, term.cur.y-1);
            tclearregion(0, term.cur.y, term.cur.x, term.cur.y);
            break;
        case 2: /* all */
    ...
    case 'K': /* EL -- Clear line */
        switch (csiescseq.arg[0]) {
        case 0: /* right */
            tclearregion(term.cur.x, term.cur.y, term.col-1,
                    term.cur.y);
            break;
        case 1: /* left */
            tclearregion(0, term.cur.y, term.cur.x, term.cur.y);
            break;
        case 2: /* all */
            tclearregion(0, term.cur.y, term.col-1, term.cur.y);
            break;
        }
        break;
    ...
    case 'X': /* ECH -- Erase <n> char */
        DEFAULT(csiescseq.arg[0], 1);
        tclearregion(term.cur.x, term.cur.y,
                term.cur.x + csiescseq.arg[0] - 1, term.cur.y);
        break;
    ...
    case 'd': /* VPA -- Move to <row> */
        DEFAULT(csiescseq.arg[0], 1);
        tmoveato(term.cur.x, csiescseq.arg[0]-1);
        break;
...
void osc_color_response(int num, int index, int is_osc4)
{
    ...
    char buf[32];
    int n = snprintf(buf, sizeof buf, "\033]%s%d;rgb:%02x%02x/%02x%02x/%02x%02x\007",
...
void tdump(void)
{
    for (int i = 0; i < term.row; ++i) tdumpline(i);
}
...
tcontrolcode(uchar ascii)
{
    ...
    case '\t':   /* HT */
        ...
        return;
    case '\b':   /* BS */
        tmoveto(term.cur.x-1, term.cur.y);
        return;
    case '\r':   /* CR */
        tmoveto(0, term.cur.y);
        return;
    case '\f':   /* LF */
    case '\v':   /* VT */
    case '\n':   /* LF */
    ...
    case '\017': /* SI (LS0 -- Locking shift 0) */
        ...
        return;
    case '\032': /* SUB */
        tsetchar('?', &term.cur.attr, term.cur.x, term.cur.y);
        /* FALLTHROUGH */
    case '\030': /* CAN */
        csireset();
        break;
...
eschandle(uchar ascii)
{
    ...
    case '+': /* G3D4 -- set quaternary charset G3 */
        ...
        return 0;
    case 'D': /* IND -- Linefeed */
        if (term.cur.y == term.bot) {
            tscrollup(term.top, 1);
        } else {
            tmoveto(term.cur.x, term.cur.y+1);
        }
        break;
...
check_control_code:
    ...
    if (selected(term.cur.x, term.cur.y))
        selclear();

    gp = &TLINE(term.cur.y)[term.cur.x];
    if (IS_SET(MODE_WRAP) && (term.cur.state & CURSOR_WRAPNEXT)) {
        gp->mode |= ATTR_WRAP;
        tnewline(1);
        gp = &TLINE(term.cur.y)[term.cur.x];
    }

    if (IS_SET(MODE_INSERT) && term.cur.x+width < term.col) {
        memmove(gp+width, gp, (term.col - term.cur.x - width) * sizeof(Glyph));
        gp->mode &= ~ATTR_WIDE;
    }

    if (term.cur.x+width > term.col) {
        if (IS_SET(MODE_WRAP))
            tnewline(1);
        else
            tmoveto(term.col - width, term.cur.y);
        gp = &TLINE(term.cur.y)[term.cur.x];
    }

    tsetchar(u, &term.cur.attr, term.cur.x, term.cur.y);
    term.lastc = u;

    if (width == 2) {
        gp->mode |= ATTR_WIDE;
        if (term.cur.x+1 < term.col) {
            if (gp[1].mode == ATTR_WIDE && term.cur.x+2 < term.col) {
                gp[2].u = ' ';
    ...
    if (term.cur.x+width < term.col) {
        tmoveto(term.cur.x+width, term.cur.y);
    } else {
        term.cur.state |= CURSOR_WRAPNEXT;
    }
}
...
void t_resize(int col, int row)
{
    ...
    for (int i = term.row; i < row; ++i) {
        ...
        clearline(term.screen[1].buffer[i], term.cur.attr, 0, linelen);
    }
...
draw(void)
{
    int cx = term.cur.x, ocx = term.ocx, ocy = term.ocy;
    ...
    if (TLINE(term.ocy)[term.ocx].mode & ATTR_WDUMMY)
        term.ocx--;
    if (TLINE(term.cur.y)[cx].mode & ATTR_WDUMMY)
        cx--;

    drawregion(0, 0, term.col, term.row);
    if (TSCREEN.off == 0)
        xdrawcursor(cx, term.cur.y, TLINE(term.cur.y)[cx],
